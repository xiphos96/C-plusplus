...
void Matrix4x4Intrinsic2( float* MatrixA, __m128* MatrixB, float* MaxtrixDest)
{
	int indexA = 0;
	int indexB = 0;
...
void Matrix4x4Vector( float* MatrixA, float* MatrixB, float* MaxtrixDest)
{
	int indexA = 0;
	int indexB = 0;
	...
	for( i = 0; i < 4; i++)
		loadu(xmmB[i],(MatrixB+(i*4)));

	for( i = 0; i < 4; i++)
	{
		indexB = 0;

		xmmR = F32vec4(MatrixA[indexA++])*xmmB[indexB++]
			+ F32vec4(MatrixA[indexA++])*xmmB[indexB++]
			+ F32vec4(MatrixA[indexA++])*xmmB[indexB++]
			+ F32vec4(MatrixA[indexA++])*xmmB[indexB++];

		storeu(MaxtrixDest+(i*4),xmmR);
	}
}

void Matrix4x4Vector2( float* MatrixA, F32vec4* MatrixB, float* MaxtrixDest)
{
	int indexA = 0;
	int indexB = 0;
...
int main(int argc, char* argv[])
{
	...
	watch.End();
	printf("4x4 Assembly SIMD Time : %f\n",watch.GetDurationMilliSecond());
	...
	printf("4x4 Intrinsic SIMD Time : %f\n",watch.GetDurationMilliSecond());
	
	watch.Start();
	for(i = 0; i < 10000; i++)
		Matrix4x4Intrinsic2(MatrixA,(__m128*)MatrixB,MatrixDest);
	watch.End();
	
	printf("4x4 Intrinsic2 SIMD Time : %f\n",watch.GetDurationMilliSecond());

	watch.Start();
	for(i = 0; i < 10000; i++)
		Matrix4x4Vector2(MatrixA,(F32vec4*)MatrixB,MatrixDest);
	watch.End();
	
	printf("4x4 Vector2 SIMD Time : %f\n",watch.GetDurationMilliSecond());
