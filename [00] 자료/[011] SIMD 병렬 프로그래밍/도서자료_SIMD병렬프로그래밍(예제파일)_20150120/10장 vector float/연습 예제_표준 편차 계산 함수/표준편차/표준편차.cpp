...
float GetStdevVector(float* Array, int Size)
{
	...
	F32vec4 xmmSum(0.0);
	F32vec4 xmmCur(0.0);
	F32vec4 xmmResult(0.0);
	
	int i = 0;

	for( i = 0; i < LoopSize; i+=4)
	{
		loadu( xmmCur, Array+i );	//4개씩 연속으로 읽어 들인다.
		xmmSum += xmmCur;	//4개씩 누적합을 구한다.
	}
	
	//Package안 4개의 합을 하나로 구한다.
	xmmResult = _mm_shuffle_ps(xmmResult, xmmSum, 0x40);
	xmmSum += xmmResult;
	xmmResult = _mm_shuffle_ps(xmmResult, xmmSum, 0x30);
	xmmSum += xmmResult;
	...
	F32vec4 xmmDistribution(0.0);
	F32vec4 xmmSumDist(0.0);
	
	for(  i = 0; i < LoopSize; i+=4)
	{
		loadu( xmmCur, Array+i );	//4개씩 연속으로 읽어 들인다.
		xmmDistribution = xmmAVG - xmmCur;	//평균값에서 뺀다.
		xmmDistribution = xmmDistribution * xmmDistribution;	//제곱을 구한다.
		xmmSumDist += xmmDistribution;	//편차 누적합을 구한다.
	}
	
	xmmResult = F32vec4(0.0);
	Sum = 0;
