...
public:
    ...
    {
        char buffer[256];
        snprintf(buffer, sizeof(buffer), "%d", mValue);
...
public:
    ...
    {
        char buffer[256];
        snprintf(buffer, sizeof(buffer), "%f", mValue);
        return std::string(buffer);
    }
...
Arg::Arg(const Arg &other) :
    mImpl(other.mImpl ? other.mImpl->Clone() : nullptr)
{
}

Arg &Arg::operator=(const Arg &other)
{
    Arg::Base *newval = other.mImpl ? other.mImpl->Clone() : nullptr;
    delete mImpl;
    mImpl = newval;
    return *this;
}
...
bool Arg::Empty() const { return (mImpl == nullptr); }
