...
// Exer
ScoreManager GameApp::score(0);

// Exer
ScoreManager::ScoreManager(int s)
{
	totalScore = s;
}

void ScoreManager::plusScore(int s)
{
	totalScore += s;
}

void ScoreManager::minusScore(int s)
{
	totalScore -= s;
}

void ScoreManager::printScore(Application& win)
{
	win.wout << setpos(500, 20) << TEXT("점수: ");
	win.wout << setpos(550, 20) << totalScore;
}
