...
public:
		...
		: size(original.size), ptr(original.ptr), serial_number(++counter)
	{
		...
		cout << "배열 " << serial_number << " move 생성" << endl;
	}
	...
	Array& operator=(Array&& original)
	{
		...
		cout << "배열 " << serial_number << "에 배열 "
			<< original.serial_number << " move 배정" << endl;
	}
	...
	Array operator+(const Array& rhs)
	{
		if (size != rhs.get_size())
			throw ArrayException("좌측항과 우측항이 같은 크기가 아님!");
		Array result(size);
		for (int i = 0; i < size; i++)
...
int main()
{
	try {
		...
		for (int i = 0; i < 5; i++)
			a[i] = i;
		printem(a);
		cout << "a 배열로 초기화된 b 배열" << endl;
		Array b(a);
		printem(b);
		// Exer
		cout << "rvalue 표현식(a + b)로 초기화된 c 배열" << endl;
		Array c(a + b);
		printem(c);
		cout << "default 생성 후 rvalue 표현식(a + b)으로 초기화된 d 배열" << endl;
		Array d;
		d = a + b;
		printem(d);
	}
