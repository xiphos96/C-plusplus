...
int main()
{
	...
	for (CoinMoney c : coinVec)
		cout << c;
	// first CoinMoney whose value is greater than...
	vector<CoinMoney>::iterator it;
//	FindGreaterValue functor;
//	functor.compareValue = 2400;
	// Exer: lambda
	const int compareValue = 2400;
	it = find_if(coinVec.begin(), coinVec.end(), [&compareValue](const CoinMoney& m) {
		return (m.get_value() >= compareValue ? true : false);
	});
	if (it != coinVec.end()) {
		cout << compareValue << "원보다 큰 첫 동전다발은 " << *it;
	}
	else {
		cout << compareValue << "원보다 큰 동전다발은 없음" << endl;
	}
