...
class ATL_NO_VTABLE CShape : 
	...
	public IPerPropertyBrowsingImpl<CShape>,
...
BEGIN_PROP_MAP(CShape)
	...
	PROP_ENTRY("ShapeType", 1, __uuidof(CShapeProp))
...
public:
	...
	STDMETHOD(GetDisplayString)(DISPID dispID,BSTR *pBstr)
	{
		if(dispID == 1) {
			switch(m_nShapeType) {
			case 0 :
				*pBstr = ::SysAllocString(L"사각형");
				return S_OK;
			case 1 :
				*pBstr = ::SysAllocString(L"원형");
				return S_OK;
			case 2 :
				*pBstr = ::SysAllocString(L"삼각형");
				return S_OK;
			}
		}
		*pBstr = NULL;
									...
									CADWORD *pCaCookiesOut)
	{
		...
		pCaStringsOut->pElems = 
			(LPOLESTR*)CoTaskMemAlloc(sizeof(LPOLESTR) * 3);		
		if (pCaStringsOut->pElems == NULL)
			return E_OUTOFMEMORY;
		pCaCookiesOut->cElems = 3;
		pCaCookiesOut->pElems = 
			(DWORD*)CoTaskMemAlloc(sizeof(DWORD*) * 3);
		...
		LPOLESTR lpszCopy;
		static LPCTSTR str[3] = {"사각형", "원형", "삼각형"};
		static DWORD cookie[3] = {0, 1, 2};
	
		for (int i = 0; i < 3; i++) {
			lpszCopy = CopyString(str[i]);
			if (lpszCopy == NULL) {
				while (--i >= 0)
		...
		pCaStringsOut->pElems[i] = lpszCopy;
		pCaCookiesOut->pElems[i] = cookie[i];
		}
