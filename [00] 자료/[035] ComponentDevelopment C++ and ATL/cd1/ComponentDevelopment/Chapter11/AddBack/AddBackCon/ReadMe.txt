========================================================================
    액티브 템플릿 라이브러리 : AddBackCon 프로젝트 개요
========================================================================

사용자가 DLL(동적 연결 라이브러리)을 만드는 데 있어 출발점으로 사용하도록
응용 프로그램 마법사에서 이 AddBackCon 프로젝트를 만들었습니다.
...
_AddBackCon.idl
    이 파일은 해당 프로젝트가 빌드될 때 컴파일러에 의해 생성되며, 
    형식 라이브러리의 IDL 정의, 프로젝트에서 정의된 인터페이스와 보조 클래스가 포함됩니다.
    이 파일은 MIDL 컴파일러에 의해 처리되어 다음을 생성합니다.
        C++ 인터페이스 정의 및 GUID 선언 (_AddBackCon.h)
        GUID 정의                        (_AddBackCon_i.c)
        형식 라이브러리                  (_AddBackCon.tlb)
        마샬링 코드                      (_AddBackCon_p.c 및 dlldata.c)
AddBackCon.cpp
...
StdAfx.h 및 StdAfx.cpp는
    AddBackCon.pch라는 이름의 PCH(미리 컴파일된 헤더) 파일과
...
AddBackConps.vcproj
	...
	프록시/스텁 DLL을 빌드하는 데 필요한 dlldata.c, AddBackCon_i.c 및
	AddBackCon_p.c가 생성됩니다.
