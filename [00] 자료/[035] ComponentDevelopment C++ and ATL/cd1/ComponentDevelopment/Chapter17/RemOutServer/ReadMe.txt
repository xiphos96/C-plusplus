========================================================================
    액티브 템플릿 라이브러리 : RemOutServer 프로젝트 개요
========================================================================

사용자가 실행 파일(EXE)을 만드는 데 있어 출발점으로 사용하도록
응용 프로그램 마법사에서 이 RemOutServer 프로젝트를 만들었습니다.
...
RemOutServer.idl
    ...
    이 파일은 MIDL 컴파일러에 의해 처리되어 다음을 생성합니다.
        C++ 인터페이스 정의 및 GUID 선언 (RemOutServer.h)
        GUID 정의                                (RemOutServer_i.c)
        형식 라이브러리                                  (RemOutServer.tlb)
        마샬링 코드                                 (RemOutServer_p.c 및 dlldata.c)

RemOutServer.h
    이 파일에는 RemOutServer.idl에서 정의된 항목의 C++ 인터페이스가 정의되어 있고
    GUID가 선언되어 있으며, 컴파일하는 동안 MIDL에 의해 다시 생성됩니다.
RemOutServer.cpp
    이 파일에는 오브젝트 맵이 들어 있으며 WinMain이 구현되어 있습니다.
RemOutServer.rc
    프로그램에서 사용하는 모든
...
StdAfx.h 및 StdAfx.cpp는
    RemOutServer.pch라는 이름의 PCH(미리 컴파일된 헤더) 파일과
...
RemOutServerps.vcproj
	...
	프록시/스텁 DLL을 빌드하는 데 필요한 dlldata.c, RemOutServer_i.c 및
	RemOutServer_p.c가 생성됩니다.
