...
void CSlidingDialogDlg::OnSlidingButton() 
{
	...
	// 현재 보이는 상태이냐에 따라 버튼의 텍스트 바꾸기
	if(m_wndChild.IsVisible())
		GetDlgItem(IDC_SLIDING_BUTTON)->SetWindowText("다이얼로그 닫기");
	else
		GetDlgItem(IDC_SLIDING_BUTTON)->SetWindowText("다이얼로그 열기");
}
