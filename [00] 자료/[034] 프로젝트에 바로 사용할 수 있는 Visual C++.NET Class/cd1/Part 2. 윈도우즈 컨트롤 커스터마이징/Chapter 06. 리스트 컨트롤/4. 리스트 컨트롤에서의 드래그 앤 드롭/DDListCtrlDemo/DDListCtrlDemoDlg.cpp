...
BOOL CDDListCtrlDemoDlg::OnInitDialog()
{
	...
	// 좌측 리스트 컨트롤에 이미지 리스트를 설정한다
	m_AllList.SetImageList(&m_BigImageList, LVSIL_NORMAL);
	m_AllList.SetImageList(&m_SmallImageList, LVSIL_SMALL);

	// 우측 리스트 컨트롤에 이미지 리스트를 설정한다
	m_TargetList.SetImageList(&m_BigImageList, LVSIL_NORMAL);
	m_TargetList.SetImageList(&m_SmallImageList, LVSIL_SMALL);

	// 컬럼을 생성한다
	m_AllList.InsertColumn(0, "동물 이름", LVCFMT_LEFT, 240);
	m_TargetList.InsertColumn(0, "동물 이름", LVCFMT_LEFT, 240);

	// 좌측 리스트 컨트롤에 항목을 추가한다
	m_AllList.InsertItem(0, "물고기", 0);
	m_AllList.InsertItem(1, "파랑새", 1);
	m_AllList.InsertItem(2, "토끼", 2);
	m_AllList.InsertItem(3, "낙타", 3);
	m_AllList.InsertItem(4, "꽃게", 4);
	m_AllList.InsertItem(5, "당나귀", 5);
	m_AllList.InsertItem(6, "강아지", 6);
	m_AllList.InsertItem(7, "펜더곰", 7);
	m_AllList.InsertItem(8, "펭귄", 8);
