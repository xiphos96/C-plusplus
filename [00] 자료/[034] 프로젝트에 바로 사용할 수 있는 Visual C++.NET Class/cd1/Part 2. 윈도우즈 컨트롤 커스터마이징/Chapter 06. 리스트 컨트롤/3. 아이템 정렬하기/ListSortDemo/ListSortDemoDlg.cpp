// ListSortDemoDlg.cpp : 구현 파일
//

#include "stdafx.h"
#include "ListSortDemo.h"
#include "ListSortDemoDlg.h"
...
CListSortDemoDlg::CListSortDemoDlg(CWnd* pParent /*=NULL*/)
	: CDialog(CListSortDemoDlg::IDD, pParent)
{
	//{{AFX_DATA_INIT(CListSortDemoDlg)
	m_strName = _T("");
	m_strAge = _T("");
	m_strAddr = _T("");
	//}}AFX_DATA_INIT
...
void CListSortDemoDlg::DoDataExchange(CDataExchange* pDX)
{
	CDialog::DoDataExchange(pDX);
	//{{AFX_DATA_MAP(CListSortDemoDlg)
...
BEGIN_MESSAGE_MAP(CListSortDemoDlg, CDialog)
	//{{AFX_MSG_MAP(CListSortDemoDlg)
	ON_WM_SYSCOMMAND()
	ON_WM_PAINT()
	ON_WM_QUERYDRAGICON()
...
// CListSortDemoDlg 메시지 처리기

BOOL CListSortDemoDlg::OnInitDialog()
{
...
void CListSortDemoDlg::OnSysCommand(UINT nID, LPARAM lParam)
{
...
//  이 함수를 호출합니다. 
HCURSOR CListSortDemoDlg::OnQueryDragIcon()
{
	return static_cast<HCURSOR>(m_hIcon);
}

void CListSortDemoDlg::OnAddButton() 
{
	// 추가 버튼을 클릭하면
	...
	// 항목 추가
	// 추가시 정렬 될 것이기 때문에 InsertItem()의 리턴 값을 저장하고
	// 저장된 항목 번호를 가지고 SetItemText()를 실행해야 한다
