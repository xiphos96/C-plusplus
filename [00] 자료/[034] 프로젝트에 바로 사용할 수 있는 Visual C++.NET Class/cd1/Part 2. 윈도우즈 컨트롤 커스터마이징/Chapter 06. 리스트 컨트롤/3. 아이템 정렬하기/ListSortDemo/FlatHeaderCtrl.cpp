	...
	for(i = 0; i < nSize; i++)
	{
		...
		if(rcItem.right >= rcClip.left && rcItem.left <= rcClip.right)
		{
			...
			case HDF_LEFT:
				...
				// 텍스트 영역을 제한다
				rcItem.left += (nWidth > 0) ? nWidth + CONST_TERM : 0;
				// 만약 현재 항목이 정렬 기준 컬럼이라면
				if(m_nSortColumn == i)
				{
					// 정렬 방향 표시자를 그린다
					nWidth = DrawSortMark(&dc, &rcItem, &Item);
					// 정렬 방향 표시자의 폭을 제한다
					rcItem.left += (nWidth > 0) ? nWidth + CONST_TERM : 0;
				}
				// 텍스트의 좌측에 비트맵을 출력한다
			...
			// 만약 클릭된 항목이라면 반전 시킨다
			if(m_nClickColumn == i)			dc.InvertRect(rcItem);
...
int CFlatHeaderCtrl::DrawSortMark(CDC* pDC, CRect *pRect, LPHDITEM pItem)
{
	// 만약 표시자의 폭보다 출력 영역이 작다면 리턴
	if(pRect->Width() < CONST_SORT_MARK)		return 0;

	// 텍스트 정렬 방향에 따라
	// 만약 좌측 정렬이면 출력 영역의 좌측에 출력하고
	CRect rcMark = pRect;
	if(pItem->fmt & HDF_JUSTIFYMASK == HDF_LEFT)
		rcMark.left = pRect->left;
	else
		// 만약 중앙이나 우측 정렬이면 출력 영역의 우측에 출력한다
		rcMark.left = pRect->right - CONST_SORT_MARK;
	rcMark.right	= rcMark.left + CONST_SORT_MARK - 1;
	rcMark.top		= (pRect->top + pRect->bottom - CONST_SORT_MARK) / 2;
	rcMark.bottom	= rcMark.top + CONST_SORT_MARK - 1;

	// 방향 표시 삼각형의 가운데 수평 위치를 구한다
	int nCenterX = rcMark.left + CONST_SORT_MARK / 2;

	CPen *pOldPen = pDC->SelectObject(&m_HighLightPen);

	// 만약 오름 차순 정렬이면
	if(m_bSortAscending)
	{
		// 위쪽을 향하는 삼각형을 그린다
		pDC->MoveTo(nCenterX,		rcMark.top);
		pDC->LineTo(rcMark.right,	rcMark.bottom);
		pDC->LineTo(rcMark.left,	rcMark.bottom);
	...
	{
		// 내림 차순이라면 아래쪽을 향하는 삼각형을 그린다
		pDC->MoveTo(nCenterX,		rcMark.bottom);
		pDC->LineTo(rcMark.right,	rcMark.top);

		pDC->SelectObject(&m_ShadowPen);
		pDC->MoveTo(nCenterX,		rcMark.bottom);
		pDC->LineTo(rcMark.left,	rcMark.top);
		pDC->LineTo(rcMark.right,	rcMark.top);
	}
		
	pDC->SelectObject(pOldPen);

	return CONST_SORT_MARK;
}
...
void CFlatHeaderCtrl::RecalcToolRect()
{
	...
	for(i = 0; i < nSize; i++)
	{
		...
		{
			// 정렬 방향 표시자의 폭을 더한다
			nWidth += CONST_SORT_MARK + CONST_TERM;
		}
...
void CFlatHeaderCtrl::SetSortColumn(int nCol, BOOL bAscending)
{
	// 새 기준을 저장한다
	m_nSortColumn		= nCol;
...
void CFlatHeaderCtrl::OnLButtonDown(UINT nFlags, CPoint point) 
{
	...
	// 모든 항목에 대해, 해당 항목의 영역에 포함되는지 확인한다
	m_nClickColumn = -1;
	for(i = 0; i < nSize; i++)
	{
		// 항목의 영역을 구한다
