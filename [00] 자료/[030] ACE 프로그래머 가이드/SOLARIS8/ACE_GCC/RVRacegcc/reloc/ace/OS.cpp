// OS.cpp,v 1.8 2003/05/21 16:00:56 shuston Exp
// OS.cpp,v 4.625 2002/12/21 23:29:24 shuston Exp
...
ACE_RCSID(ace, OS, "OS.cpp,v 1.8 2003/05/21 16:00:56 shuston Exp")
  ...
  if (fp != 0)
    {
      ...
      // However, for TAO, ASCII files, such as IOR file, can still be
      // read and written without any error since given buffers are all
      // in 'char' type instead of ACE_TCHAR.  Therefore, it is user's
      // reponsibility to select correct buffer type.

      // At this point, check if the file is Unicode or not.
      ACE_UINT16 first_two_bytes;
      int numRead = ACE_OS::fread (&first_two_bytes,
                                   sizeof(ACE_UINT16),
                                   1,
                                   fp);
      if (numRead == 1)
        {
          if ((first_two_bytes != 0xFFFE) &&  // not a small endian Unicode
              (first_two_bytes != 0xFEFF))    // not a big endian Unicode
            {
              ACE_OS::fseek(fp, 0, SEEK_SET);  // set pointer back to start
            }
        }
      // if it is a Unicode file, file pointer will be right next to
      // the first two-bytes
    }
}
...
#  endif /* ACE_WIN32 */
  ...
  ACE_UNUSED_ARG (format);
  ACE_NOTSUP_RETURN (-1);
#endif /* _XOPEN version checks or ACE_WIN32 */
}
#endif /* ACE_HAS_WCHAR */
...
#  else
          ...
          if (ctemp == 0)
            ...
            buf_len += ACE_OS_String::strlen (ctemp);
