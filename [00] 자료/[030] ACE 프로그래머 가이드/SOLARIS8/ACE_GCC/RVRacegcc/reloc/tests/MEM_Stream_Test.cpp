// MEM_Stream_Test.cpp,v 4.44 2002/12/31 02:33:08 shuston Exp
...
ACE_RCSID(tests, MEM_Stream_Test, "MEM_Stream_Test.cpp,v 4.44 2002/12/31 02:33:08 shuston Exp")
...
{
  ACE_DEBUG ((LM_DEBUG, ACE_TEXT ("Testing Reactive MEM_Stream\n\n")));
  ...
  if (argc == 1)
    {
      ...
      test_reactive (ACE_LIB_TEXT("\\Windows\\Start Menu\\MEM_Stream_Test.exe"), server_addr);
#else
...
#if defined (ACE_HAS_WINCE)
      test_concurrent (ACE_LIB_TEXT("\\Windows\\Start Menu\\MEM_Stream_Test.exe"), server_addr);
#else
