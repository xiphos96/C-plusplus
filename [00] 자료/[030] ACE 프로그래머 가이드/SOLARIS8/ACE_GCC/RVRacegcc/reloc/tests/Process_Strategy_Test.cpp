// Process_Strategy_Test.cpp,v 1.4 2003/07/08 13:35:01 shuston Exp
// Process_Strategy_Test.cpp,v 4.66 2002/12/31 02:33:29 shuston Exp
...
ACE_RCSID(tests, Process_Strategy_Test, "Process_Strategy_Test.cpp,v 1.4 2003/07/08 13:35:01 shuston Exp")
...
Counting_Service::handle_input (ACE_HANDLE)
{
  ...
  if (bytes <= 0 || buf[0] == (char) EOF)
  ...
  else
    {
      buf[len] = '\0';
      ACE_DEBUG ((LM_DEBUG,
                  ACE_TEXT ("(%P|%t) %d bytes of input on %d is %*C\n"),
                  bytes,
                  this->peer ().get_handle (),
                  bytes,
                  buf));
