// Client_Test.cpp,v 1.2 2003/03/31 22:02:29 shuston Exp
...
ACE_RCSID(Client, Client_Test, "Client_Test.cpp,v 1.2 2003/03/31 22:02:29 shuston Exp")
...
private:
  ...
  int set_host (const ACE_TCHAR *hostname, int port);
...
Client_Test::handle_input (ACE_HANDLE)
{
  ...
  if (::scanf ("%s", option) <= 0)
    ACE_ERROR_RETURN ((LM_ERROR,
                       ACE_TEXT ("%p Try again!\n"),
                       ACE_TEXT ("Client_Test::handle_input")),
                      0);
  ...
  switch (isupper (option[0]) ? tolower (option[0]) : option[0])
    ...
    case 'h' :
      if (::scanf ("%s %d", buf1, &port) <= 0)
        break;
      result = this->set_host (ACE_TEXT_CHAR_TO_TCHAR (buf1), port);
      break;
    case 'b' :
      ...
      if ((temp_buf = ACE_OS::strtok (input, " ")))
        {
          ...
          if (temp_buf)
            ...
            ACE_ERROR ((LM_ERROR,
                        ACE_TEXT ("Bind Failed! Value not entered.\n")));
        }
      else
        ACE_ERROR ((LM_ERROR,
                    ACE_TEXT ("Bind Failed! Key and Value not entered.\n")));
      break;
    case 'u' :
      if (::scanf ("%s", buf1) <= 0)
        break;
      ...
      if (temp_buf)
        {
          ...
          if (temp_buf)
            ...
            ACE_ERROR ((LM_ERROR,
                        ACE_TEXT ("Rebind Failed! Value not entered.\n")));
        }
      else
        ACE_ERROR ((LM_ERROR,
                    ACE_TEXT ("Reind Failed! Key and value not entered.\n")));
      break;
    case 'f' :
      if (::scanf ("%s", buf1) <= 0)
        break;
...
int
Client_Test::set_host (const ACE_TCHAR *hostname, int port)
{
  // Close down original name space
  NAMING_CONTEXT ()->close ();
