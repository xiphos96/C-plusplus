// TS_Clerk_Handler.cpp,v 1.2 2003/03/31 22:02:31 shuston Exp
...
ACE_RCSID(lib, TS_Clerk_Handler, "TS_Clerk_Handler.cpp,v 1.2 2003/03/31 22:02:31 shuston Exp")
...
{
  ACE_TRACE (ACE_TEXT ("ACE_TS_Clerk_Handler::ACE_TS_Clerk_Handler"));
  ...
  for (HANDLER_SET_ITERATOR set_iterator (this->handler_set_);
      ...
      if ((*handler)->state () == ACE_TS_Clerk_Handler::ESTABLISHED)
	{
	  ...
	  // Check if sequence numbers match; otherwise discard
	    ...
	    {
	      count++;
	      ACE_DEBUG ((LM_DEBUG, ACE_TEXT ("[%d] Delta time: %d\n"),
      ...
      else
	{
	  ...
	  ACE_DEBUG ((LM_DEBUG,
		     ACE_TEXT ("(%t) in the process of connecting %s to %s\n"),
		      ...
		      ? ACE_TEXT ("asynchronously") : ACE_TEXT ("synchronously"), buf));
	}
    }
  else
    {
