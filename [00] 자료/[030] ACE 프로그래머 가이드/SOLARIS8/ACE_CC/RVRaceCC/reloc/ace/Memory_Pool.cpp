// Memory_Pool.cpp,v 1.5 2003/09/04 21:56:02 boris Exp
// Memory_Pool.cpp,v 4.88 2002/10/05 21:06:47 shuston Exp
...
ACE_RCSID(ace, Memory_Pool, "Memory_Pool.cpp,v 1.5 2003/09/04 21:56:02 boris Exp")
                                                 ...
                                                 off_t &map_size)
{
  ...
  // Do not try to write to the file at this point, since file handle is
  // controlled by kernel and write and read operations should not be used
  // after file is mapped.
  map_size = ACE_OS::filesize (this->mmap_.handle ());
  size_t seek_len;
    ...
    file_mode_ (file_mode)
{
  ...
  // CE can't do fixed address anytime. So, if the base is 0, reset to never
  // use a fixed address. If the caller specified a fixed address in
  // base_addr_, he's out of luck - that's not portable to CE.
  if (base_addr_ == 0)
    use_fixed_addr_ = NEVER_FIXED;
#endif /* ACE_HAS_WINCE */
  ...
  if (backing_store_name)
    {
      ...
      if (result == 0 || result == EOF)
        ...
        this->base_shm_key_ = (key_t) ACE::crc32
          (ACE_TEXT_ALWAYS_CHAR (backing_store_name));
      else
...
// ACE_Unbounded_Set uses ACE_Node<>, so it needs to be instantiated as well.
// However, ACE_Node<ACE_TCHAR*> is already done elsewhere, so we only need
// to do ACE_Node<char*> if ACE_USES_WCHAR is set.
