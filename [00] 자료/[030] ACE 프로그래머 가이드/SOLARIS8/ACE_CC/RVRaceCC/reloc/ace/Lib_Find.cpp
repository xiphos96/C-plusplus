// Lib_Find.cpp,v 1.15 2002/12/18 16:47:28 seibel_r Exp

#include "ace/Lib_Find.h"
#include "ace/Log_Msg.h"

ACE_RCSID(ace, Lib_Find, "Lib_Find.cpp,v 1.15 2002/12/18 16:47:28 seibel_r Exp")
      ...
      if (ACE_OS::strlen (searchpathname) > 0)
        {
          if (ACE_OS::strlen (searchfilename)
          ...
          else
            {
              ...
              const ACE_TCHAR *path_file_suffix_fmt =
                ACE_LIB_TEXT ("%s%s%s");
              const ACE_TCHAR *path_prefix_file_suffix_fmt =
                ACE_LIB_TEXT ("%s%s%s%s");
#else
              const ACE_TCHAR *path_file_suffix_fmt =
                ACE_LIB_TEXT ("%ls%ls%ls");
              const ACE_TCHAR *path_prefix_file_suffix_fmt =
                ACE_LIB_TEXT ("%ls%ls%ls%ls");
#endif

#if defined (ACE_HAS_BROKEN_CONDITIONAL_STRING_CASTS)
              ACE_OS::sprintf (pathname,
                               path_file_suffix_fmt,
                               searchpathname,
                               searchfilename,
...
#    else
          ...
          ACE_Ascii_To_Wide wide_ldpath (ACE_OS::getenv (ACE_LD_SEARCH_PATH));
...
#if defined (ACE_WIN32) || !defined (ACE_USES_WCHAR)
              const ACE_TCHAR *path_file_suffix_fmt =
                ACE_LIB_TEXT ("%s%lc%s%s");
              const ACE_TCHAR *path_prefix_file_suffix_fmt =
                ACE_LIB_TEXT ("%s%lc%s%s%s");
#else
              const ACE_TCHAR *path_file_suffix_fmt =
                ACE_LIB_TEXT ("%ls%lc%ls%ls");
              const ACE_TCHAR *path_prefix_file_suffix_fmt =
                ACE_LIB_TEXT ("%ls%lc%ls%ls%ls");
#endif
              int result = 0;
