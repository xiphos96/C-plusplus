// High_Res_Timer.cpp,v 1.3 2003/07/22 01:52:47 boris Exp
// High_Res_Timer.cpp,v 4.92 2002/12/18 13:11:12 schmidt Exp
...
ACE_RCSID(ace, High_Res_Timer, "High_Res_Timer.cpp,v 1.3 2003/07/22 01:52:47 boris Exp")
...
ACE_High_Res_Timer::get_cpuinfo (void)
{
  ...
  FILE *cpuinfo = ACE_OS::fopen
    (ACE_LIB_TEXT ("/proc/cpuinfo"), ACE_LIB_TEXT ("r"));

  if (cpuinfo != 0)
    {
      // Betting that cpuinfo is always narrow-char...
      char buf[128];
          ...
          if (::sscanf (buf, "cpu : %s\n", arg) == 1)
            {
              ...
              // usable...
              if (ACE_OS::strncmp (arg, "Alpha", 5) == 0)
                {
                  supported = 1;
...
ACE_High_Res_Timer::get_env_global_scale_factor (const ACE_TCHAR *env)
{
  ...
  if (env != 0)
    {
#  if defined (ACE_WIN32)   /* Only Windows supports wide-char env var. */
