// Get_Opt_Test.cpp,v 1.8 2002/08/30 03:17:56 shuston Exp
...
ACE_RCSID(tests, Get_Opt_Test, "Get_Opt_Test.cpp,v 1.8 2002/08/30 03:17:56 shuston Exp")
            ...
            const ACE_TString &opt_prefix = empty)
{
  ...
  ACE_DEBUG ((LM_INFO,
              ACE_TEXT (" TEST %d   ************************************************************\n"),
              test_number));
  ACE_DEBUG ((LM_INFO, ACE_TEXT ("   Command line: \"%s\"\n"), test.c_str ()));
  ...
  ACE_DEBUG ((LM_INFO, ACE_TEXT ("      optstring: \"%s\"  skip_argv0: %d\n"),
  ...
  while ((c = get_opt ()) != EOF)
    {
      switch (c)
        ...
        case 0:
          ...
          ACE_DEBUG ((LM_INFO,
                      ACE_TEXT ("   Found long option \"%s\" %s %s\n"),
                      get_opt.long_option (),
        ...
        case 's':
          ...
          ACE_DEBUG ((LM_INFO, ACE_TEXT ("   Found option flag '%c'\n"), c));
          break;
        case 'r':
          ...
          ACE_DEBUG ((LM_INFO,
                      ACE_TEXT ("   Found option '%c' with required argument \"%s\"\n"),
                      c, get_opt.opt_arg ()));
          break;
        case 'o':
        ...
        case 'l':
          ...
          ACE_DEBUG ((LM_INFO,
                      ACE_TEXT ("   Found option '%c' with optional argument \"%s\"\n"),
        ...
        case 1:
          ...
          ACE_DEBUG ((LM_INFO,
                      ACE_TEXT ("   Found a non-option argument \"%s\" before")
                      ACE_TEXT (" finding \"--\" ")
                      ACE_TEXT ("(must be in RETURN_IN_ORDER mode).\n"),
                      get_opt.opt_arg ()));
          break;
        case -10:
          // we found the short option that isn't alpha numeric.
          ACE_DEBUG ((LM_INFO,
                      ACE_TEXT ("   Found option '%d' with optional argument \"%s\"\n"),
                      c, get_opt.opt_arg () ? get_opt.opt_arg ()
                                            : ACE_TEXT ("default")));
          break;
        case ':':
        ...
        default:
          ...
          ACE_DEBUG ((LM_INFO,
                      ACE_TEXT ("  Found an unknown option '%c' (%s) we couldn't handle.\n"),
...
run_test (int& test_number, int ordering)
{
  ...
  ACE_DEBUG ((LM_INFO,
              ACE_TEXT (" ##########   Running Tests with ordering = %s   ##########\n"),
              ordering == 1 ? ACE_TEXT ("REQUIRE_ORDER") :
                              ordering == 2 ? ACE_TEXT ("PERMUTE_ARGS") :
                                              ACE_TEXT ("RETURN_IN_ORDER")));
