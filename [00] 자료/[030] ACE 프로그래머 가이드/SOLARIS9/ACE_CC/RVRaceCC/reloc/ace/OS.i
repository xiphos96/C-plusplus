// -*- C++ -*-
// OS.i,v 1.11 2003/07/31 03:47:20 boris Exp
// OS.i,v 4.1075 2002/12/21 23:29:24 shuston Exp
...
# else /* ACE_WIN32 */
#   if defined (ACE_USES_WCHAR)
  // For the wide-char case, convert the wide-char input to a narrow-char
  // string for mktemp(), then convert the name back to wide-char for the
  // caller.
  ACE_Wide_To_Ascii narrow_s (s);
  if (::mktemp (narrow_s.char_rep ()) == 0)
    return 0;
  ACE_Ascii_To_Wide wide_s (narrow_s.char_rep ());
  ACE_OS_String::strcpy (s, wide_s.wchar_rep ());
  return s;
#   else
  return ::mktemp (s);
#   endif /* ACE_USES_WCHAR */
...
#  if defined (ACE_USES_WCHAR)
  // For the wide-char case, convert the wide-char input to a narrow-char
  // string for mkstemp(), then convert the name back to wide-char for the
  // caller.
  ACE_Wide_To_Ascii narrow_s (s);
  int fd;
  if ((fd = ::mkstemp (narrow_s.char_rep ())) == -1)
    return -1;
  ACE_Ascii_To_Wide wide_s (narrow_s.char_rep ());
  ACE_OS_String::strcpy (s, wide_s.wchar_rep ());
  return fd;
...
#   endif /* ACE_USES_WCHAR */
#else /* VXWORKS */
  ACE_OSCALL_RETURN (::stat (ACE_TEXT_ALWAYS_CHAR (file), stp), int, -1);
#endif /* VXWORKS */
}
...
# else
  ACE_OSCALL_RETURN (::unlink (ACE_TEXT_ALWAYS_CHAR (path)), int, -1);
# endif /* VXWORKS */
}
...
# else /* ACE_LACKS_RENAME */
  ACE_UNUSED_ARG (flags);
  ACE_OSCALL_RETURN (::rename (ACE_TEXT_ALWAYS_CHAR (old_name),
                               ACE_TEXT_ALWAYS_CHAR (new_name)),
                     int, -1);
...
#else /* VXWORKS */
  char *name = 0;
  ACE_OSCALL (::tempnam (ACE_TEXT_ALWAYS_CHAR (dir),
                         ACE_TEXT_ALWAYS_CHAR (pfx)),
              char *, 0, name);
  if (name == 0)
    return 0;
#  if defined (ACE_USES_WCHAR)
  // tempnam() returns a malloc()'d area holding the generated name. We
  // need to convert this back to a wide-char form, also malloc()'d, and
  // free the one tempnam() allocated. Then the caller still frees the
  // returned pointer as expected.
  wchar_t *w_name = 0;
  ACE_Ascii_To_Wide wide_name (name);
  free (name);
  size_t needed = (ACE_OS_String::strlen (wide_name.wchar_rep ()) + 1) *
    sizeof (wchar_t);
  if ((w_name = ACE_static_cast (wchar_t *, malloc (needed))) == 0)
    return 0;
  ACE_OS::strcpy (w_name, wide_name.wchar_rep ());
  return w_name;
#  else
  return name;
#  endif /* ACE_USES_WCHAR */
...
# if defined (ACE_HAS_SHM_OPEN)
  ACE_UNUSED_ARG (sa);
  ACE_UNUSED_ARG (share);
  ACE_OSCALL_RETURN (::shm_open (filename, mode, perms), ACE_HANDLE, -1);

# elif defined (ACE_HAS_WINCE)

  // If the file exists, hen just forward to ACE_OS::open(). If it doesn't,
  // then use CreateFileForMapping to create it.
  //  if (GetFileAttributes (filename) != 0xFFFFFFFF)
  //    return ACE_OS::open (filename, mode, perms, sa);

  // This mode-mapping code was lifted from ACE_OS::open(). It should be
  // pulled out to a separate method but API constraints within the 5.3
  // series argue against doing that here.
...
#if 0
  // This may not be completely correct... if not, this arrangement between
  // ACE_OS::shm_open() and ACE_Mem_Map::open() may need to be refactored.
  DWORD shared_mode = 0;
  if (ACE_BIT_ENABLED (share, MAP_SHARED))
    {
      if (ACE_BIT_ENABLED (access, GENERIC_READ))
        shared_mode |= FILE_SHARE_READ;
      if (ACE_BIT_ENABLED (access, PAGE_READWRITE))
        shared_mode |= FILE_SHARE_WRITE;
    }
#else
  DWORD shared_mode = 0;
  if (ACE_BIT_ENABLED (share, MAP_PRIVATE))
    shared_mode = 0;
  ...
  ACE_HANDLE h = CreateFileForMapping (filename,
                                       access,
                                       shared_mode,
                                       0,
                                       creation,
  ...
  if (h == ACE_INVALID_HANDLE)
    ACE_FAIL_RETURN (ACE_INVALID_HANDLE);   // Map GetLastError to errno
  else
    return h;
...
# if (defined (_XOPEN_SOURCE) && (_XOPEN_SOURCE >= 500)) || \
  ...
  ACE_OSCALL_RETURN (::vswprintf (buffer, ULONG_MAX, format, argptr), int, -1);

# elif defined (ACE_WIN32)
  // Windows has vswprintf, but the signature is from the older ISO C
  // standard. Also see ACE_OS::snprintf() for more info on this.

  ACE_OSCALL_RETURN (::vswprintf (buffer, format, argptr), int, -1);
...
{
  ACE_OS_TRACE ("ACE_OS::fgets(char)");
...
{
  ACE_OS_TRACE ("ACE_OS::fgets(whcar_t)");
...
ACE_OS::fopen (const ACE_TCHAR *filename, const ACE_TCHAR *mode)
{
  ...
  ACE_OSCALL_RETURN (::fopen (ACE_TEXT_ALWAYS_CHAR (filename),
...
#else
  ACE_OSCALL_RETURN (::freopen (ACE_TEXT_ALWAYS_CHAR (filename),
                                ACE_TEXT_ALWAYS_CHAR (mode),
                                stream),
                     FILE *, 0);
                  ...
                  off_t offset)
{
  ...
  ACE_OSCALL_RETURN (::truncate (ACE_TEXT_ALWAYS_CHAR (filename), offset),
                     int, -1);
#else
...
#else /* ACE_WIN32 */
  ACE_OSCALL_RETURN (::puts (ACE_TEXT_ALWAYS_CHAR (s)), int, -1);
#endif /* ACE_WIN32 && ACE_USES_WCHAR */
}
...
#  endif /* _M_UNIX */
  if (err == 0)
    return 0;
#  if defined (ACE_USES_WCHAR)
  ACE_OS_String::strncpy (buf, ACE_TEXT_CHAR_TO_TCHAR (err), BufLen);
  return buf;
#  else
  ACE_UNUSED_ARG (buf);
  return ACE_const_cast (char *, err);
#  endif /* ACE_USES_WCHAR */
#elif defined (__hpux)    /* This is only for pre-11.00 HP-UX */
  ACE_OSCALL_RETURN (::strerror (errno), char *, 0);
#elif defined (ACE_WIN32)
#  if defined (ACE_HAS_PHARLAP)
  ACE_OS::sprintf (buf, ACE_LIB_TEXT ("error code %d"), GetLastError());
#  else
...
#   if defined (ACE_HAS_SGIDLADD)
  ACE_OSCALL (::sgidladd (ACE_TEXT_ALWAYS_CHAR (filename), mode),
              void *, 0, handle);
#   elif defined (_M_UNIX)
  ACE_OSCALL (::_dlopen (ACE_TEXT_ALWAYS_CHAR (filename), mode),
              void *, 0, handle);
#   else
  ACE_OSCALL (::dlopen (ACE_TEXT_ALWAYS_CHAR (filename), mode),
              void *, 0, handle);
...
# elif defined (ACE_PSOS)
  ...
  ACE_NOTSUP_RETURN (0);
# else
  ACE_OSCALL_RETURN (::fdopen (handle, ACE_TEXT_ALWAYS_CHAR(mode)), FILE *, 0);
# endif /* ACE_HAS_WINCE */
}
...
ACE_INLINE int
ACE_OS::fdetach (const ACE_TCHAR *file)
{
  ACE_OS_TRACE ("ACE_OS::fdetach");
#if defined (ACE_HAS_STREAM_PIPES)
  ACE_OSCALL_RETURN (::fdetach (ACE_TEXT_ALWAYS_CHAR (file)), int, -1);
#else
  ACE_UNUSED_ARG (file);
...
ACE_INLINE int
ACE_OS::fattach (int handle, const ACE_TCHAR *path)
{
  ACE_OS_TRACE ("ACE_OS::fattach");
#if defined (ACE_HAS_STREAM_PIPES)
  ACE_OSCALL_RETURN (::fattach (handle, ACE_TEXT_ALWAYS_CHAR (path)), int, -1);
#else
  ACE_UNUSED_ARG (handle);
  ACE_UNUSED_ARG (path);
...
#  if defined (ACE_USES_WCHAR)
  ...
  char *result = 0;
  ACE_OSCALL (::getcwd (narrow_buf, size), char *, 0, result);
  ACE_Ascii_To_Wide wide_buf (result);
  delete [] narrow_buf;
  if (result != 0)
      ACE_OS_String::strsncpy (buf, wide_buf.wchar_rep (), size);
...
#else
  ACE_OSCALL_RETURN (::mkdir (ACE_TEXT_ALWAYS_CHAR (path), mode), int, -1);
#endif /* ACE_PSOS_LACKS_PHILE */
}
...
{
  ACE_OS_TRACE ("ACE_OS::putenv(char)");
  ...
  ACE_OSCALL_RETURN (::putenv (ACE_const_cast (char *, string)), int, -1);
#endif /* ACE_HAS_WINCE */
}
...
{
  ACE_OS_TRACE ("ACE_OS::putenv(wchar_t)");
#if defined (ACE_HAS_WINCE) || defined (ACE_PSOS)
  // WinCE and pSOS don't have the concept of environment variables.
  ACE_UNUSED_ARG (string);
  ACE_NOTSUP_RETURN (-1);
...
#else /* Has env, but doesn't work on wide-char */
  // This technically could be done, but it would involve allocating wide-char
  // memory to hold the translated value, and leaking the memory. The
  // string passed to putenv() becomes part of the environment, so it can't
  // be changed. Thus, ACE_TEXT_ALWAYS_CHAR can't be used here.
  ACE_UNUSED_ARG (string);
  ACE_NOTSUP_RETURN (0);
#endif /* ACE_HAS_WINCE */
}
