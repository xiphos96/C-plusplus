 ...
 *  @file   OS.h
 *
 *  OS.h,v 1.7 2003/07/08 13:35:01 shuston Exp
 *  OS.h,v 4.1192 2002/12/31 22:52:53 shuston Exp
 *
...
#   if defined (ACE_HAS_WCHAR) && defined (ACE_WIN32)
  ...
  static int putenv (const wchar_t *string);
...
# endif /* ACE_LACKS_CUSERID */
  ...
  static int fcntl (ACE_HANDLE handle,
                    int cmd,
                    long arg = 0);
  static int fdetach (const ACE_TCHAR *file);
...
{ \
  ...
  DWORD exelen = GetModuleFileName (hInstance, &cmdline[1], 1023); \
  cmdline[exelen+1] = ACE_TEXT ('\0'); \
  if (exelen == 0) ACE_OS::strcpy (cmdline, ACE_TEXT ("program ")); \
  else { ACE_OS::strcat (cmdline, ACE_TEXT ("\" ")); ACE_OS::strcat (cmdline, lpCmdLine); } \
