// Process.cpp,v 1.3 2003/07/08 13:35:01 shuston Exp
// Process.cpp,v 4.101 2002/10/27 03:02:07 nanbor Exp
...
ACE_RCSID (ace, Process, "Process.cpp,v 4.101 2002/10/27 03:02:07 nanbor Exp")
...
ACE_Process::spawn (ACE_Process_Options &options)
{
  ...
  // Note that WinCE's version of CreateProcess must have a module name passed
  // in (this is the first argument) while the second (command line buf) need
  // not be specified. This is different from desktop Windows. The
  // process_name() call will extract argv[0] if a command line is set and
  // no process name has been specified.
  // Further, the command line we pass to the second arg should not have the
  // module name included. So, we need to argv-ize the command line to find
  // argv[0] and get rid of it, then build a new command string with whatever
  // is left.
  ACE_TCHAR * const *argv = options.command_line_argv ();
  ACE_TString cmdline;
      ...
      while (*argv != 0)
        {
          cmdline += *argv;
          if (*(++argv) != 0)
            cmdline += ACE_LIB_TEXT (" ");
        }
    }
  BOOL fork_result =
    ACE_TEXT_CreateProcess (options.process_name (),
                            cmdline.c_str (),
                            0, // options.get_process_attributes() MBZ in CE
                            0, // options.get_thread_attributes()  MBZ in CE
                            0, // options.handle_inheritence()     MBZ in CE
                            options.creation_flags(),
                            0, // options.env_buf()                MBZ in CE
                            0, // options.working_directory()      MBZ in CE
                            0, // options.startup_info()           MBZ in CE
                            &this->process_info_);
  ...
  ACE_FAIL_RETURN (ACE_INVALID_PID);
    ...
    process_group_ (ACE_INVALID_PID)
{
  ACE_NEW (command_line_buf_,
           ACE_TCHAR[cobl]);
  command_line_buf_[0] = ACE_TEXT ('\0');

#if !defined (ACE_HAS_WINCE)
  working_directory_[0] = ACE_TEXT ('\0');
  ACE_NEW (environment_buf_,
           ACE_TCHAR[ebl]);
  ACE_NEW (environment_argv_,
           ACE_TCHAR *[mea]);
  environment_buf_[0] = ACE_TEXT ('\0');
  ...
  if (command_line_argv_calculated_ == 0)
    {
      ...
      parser.delimiter_replace (ACE_LIB_TEXT (' '), ACE_LIB_TEXT ('\0'));
      parser.preserve_designators (ACE_LIB_TEXT ('\"'), ACE_LIB_TEXT ('\"'));
      parser.preserve_designators (ACE_LIB_TEXT ('\''), ACE_LIB_TEXT ('\''));
