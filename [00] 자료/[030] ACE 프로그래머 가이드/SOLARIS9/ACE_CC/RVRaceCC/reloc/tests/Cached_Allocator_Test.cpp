// Cached_Allocator_Test.cpp,v 1.3 2003/05/21 16:01:03 shuston Exp
// Cached_Allocator_Test.cpp,v 4.7 2002/08/23 22:34:13 shuston Exp
...
ACE_RCSID(tests, Cached_Allocator_Test, "Cached_Allocator_Test.cpp,v 1.3 2003/05/21 16:01:03 shuston Exp")
...
ACE_TMAIN (int argc, ACE_TCHAR *argv[])
{
  ...
  const ACE_TCHAR *str1 = ACE_TEXT ("12345678");
  const ACE_TCHAR *str3 = ACE_TEXT ("ABCDEFGH");
  ...
  string_size = ACE_OS_String::strlen (str1);
  chunk_size = string_size * sizeof (ACE_TCHAR);
  n_chunks = 2;
  ...
  ptr1 = (ACE_TCHAR *) allocator.malloc (requested_size);
  if (!ptr1)
  ...
  ptr2 = (ACE_TCHAR *) allocator.malloc (requested_size);
  if (!ptr2)
  ...
  ACE_DEBUG ((LM_INFO,
              ...
              requested_size));
  ptr3 = (ACE_TCHAR *) allocator.malloc (requested_size);
  if (!ptr3)
    ACE_ERROR_RETURN ((LM_ERROR, ACE_TEXT (" (%t) Failed, exiting.\n")), -1);
  ...
  ptr4 = (ACE_TCHAR *) allocator.malloc (requested_size);
  if (!ptr4)
    ACE_DEBUG ((LM_INFO, ACE_TEXT (" (%t) OK, failed.\n")));
  else
    ACE_ERROR_RETURN ((LM_ERROR, ACE_TEXT (" (%t) Something is wrong\n")), -1);
