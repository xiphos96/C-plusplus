// Mem_Map.cpp,v 1.5 2003/09/09 19:16:26 shuston Exp
// Mem_Map.cpp,v 4.36 2002/10/05 00:25:52 shuston Exp
...
ACE_RCSID(ace, Mem_Map, "Mem_Map.cpp,v 1.5 2003/09/09 19:16:26 shuston Exp")
                     ...
                     LPSECURITY_ATTRIBUTES sa)
{
  ...
  if (remap_)
  {
      // If we are remapping (extending memory/file) we have to re-open file,
      // since file handle is automatically closed after memory is unmapped and
      // mapping handle closed. In WinCE kernel controls file handle and user
      // should not use read and write operations after file is mapped.
      handle = ACE_OS::shm_open (filename_, flags_, mode_, NULL, share_);
  }
#endif
...
#if defined (ACE_HAS_WINCE)
  // Since WinCE apps that open map file as second applications with
  // CreateFileMapping instead CreateFileForMapping this is legal
  if (result == -1)
  {
      result = 0;
  }
  ...
  if (length_request == -1)
  ...
  else
    {
      ...
      if (requested_file_length > current_file_length)
        {
          // If we are in WinCE, do not close mapping, since that would
          // invalidate file handle and mapping would fail.
#if !defined (ACE_HAS_WINCE)
                   ...
                   int share)
{
  ...
  flags_ = flags;
  if (ACE_BIT_ENABLED(flags_, _O_CREAT))
  {
      ACE_CLR_BITS(flags_, _O_CREAT);
  }
  if (ACE_BIT_ENABLED (flags_, _O_TRUNC))
  {
      ACE_CLR_BITS(flags_, _O_TRUNC);
  }
  mode_ = mode;
  share_ = share;
  this->handle_ = ACE_OS::shm_open (file_name, flags, mode, sa, share);
#else
                  ...
                  LPSECURITY_ATTRIBUTES sa)
{
  ...
  {
#if defined (ACE_HAS_WINCE)
      // In case of WinCE open can fail second time as well, since only one app
      // can call CreateFileForMapping and all other should call CreateFileMapping,
      // which well call through map_it.
      if (::GetLastError () == ERROR_SHARING_VIOLATION)
      {
          // CreateFileMapping requires handle to be invalid.
          this->handle_ = (HANDLE)INVALID_HANDLE_VALUE;
