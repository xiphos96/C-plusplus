/* -*- C++ -*- */
// Peer.h,v 1.2 2003/05/14 23:16:30 shuston Exp
// Peer.h,v 4.19 2002/07/09 15:15:08 shuston Exp
 ...
 * Class: Stdin_Reader
 *
 * This class processes stdin input for a specified Peer_Handler.
 * The input is events to send off to the gateway. When EOF is read,
 * the Peer_Handler's handle_close() is called as well, to shut it
 * all down.
 */
class Stdin_Reader : public ACE_Event_Handler
{
public:
  Stdin_Reader (Peer_Handler *my_peer);

  void start (void);    // Tell this guy to start reading stdin
...
private:
  Stdin_Reader () {}    // Need to specify the Peer_Handler

  Peer_Handler *peer_;
  int working_;         // Non-zero if we're working on stdin
};

class ACE_Svc_Export Peer_Handler : public ACE_Svc_Handler<ACE_SOCK_STREAM, ACE_NULL_SYNCH>
{
  friend class Stdin_Reader;
